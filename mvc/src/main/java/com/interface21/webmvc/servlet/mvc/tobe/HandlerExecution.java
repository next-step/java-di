...
import com.interface21.webmvc.servlet.mvc.tobe.support.ArgumentResolvers;
...
public class HandlerExecution {
  ...
  public HandlerExecution(final ArgumentResolvers argumentResolvers,
  ...
  private MethodParameter[] getMethodParameters() {
    ...
    if (methodParameters == null) {
      ...
      for (int i = 0; i < methodParameters.length; i++) {
        methodParameters[i] = new MethodParameter(method, parameterTypes[i],
            parameterAnnotations[i], parameterNames[i]);
      }
  ...
  private Object getArguments(MethodParameter methodParameter, HttpServletRequest request,
      HttpServletResponse response) {

    return argumentResolvers.resolveArgument(methodParameter, request, response);
  }
}
